...
interface MoonInfo {
  ...
  phaseName: string;    // 月相名（日本語）
  phaseEmoji: string;   // 月相の絵文字
  illumination: number; // 輝面比（0〜1）
  phaseIndex: number;   // 0-7の月相インデックス
}
...
function drawMoon(svg: SVGElement, info: MoonInfo): void {
  const size = 200;
  const cx = size / 2;
  const cy = size / 2;
  const r = 80;
...
function renderCalendar(year: number, month: number, container: HTMLElement, selectedDate?: string): void {
  ...
  container.querySelectorAll('.moon-day').forEach(td => {
    ...
    td.addEventListener('keydown', (e: Event) => {
...
function updateDisplay(): void {
  ...
  const svgEl = document.getElementById('moon-svg') as unknown as SVGElement;
